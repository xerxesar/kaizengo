...
| `packages/sdk-solid` | `@kaizengo/sdk-solid/...` | UI kit, spa-config, identity/search clients & components |
...
3. Optionally add a SPA — GraphQL CRUD is generated from the spec; UI from `@kaizengo/sdk-solid/ui`
...
| `views/*.page.tsx` | Svelte pages menus mount (`view: Name`) |
...
```svelte
<!-- apps/settings/spa/App.tsx — views only, no LayoutMenu -->
<script>
...
Helpers: `fetchAppMenus`, `navigateApp`, `contentAppForMenu`, `SpaMountContext` from `@kaizengo/sdk-solid/ui`.
