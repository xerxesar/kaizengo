...
This section is for the rest: **read and write models from Go**, wrap that in a **service** with extra GraphQL, and ship **Svelte pages** that are not a table or a form.
...
Work through the pages in order. Keep `make dev` running; restart it when Go files change. Svelte under `apps/todo/views/` hot-reloads.

## When to leave the spec

| Need | Stay on YAML | Write Go / custom Svelte |
|------|----------------|--------------------------|
...
2. [Custom pages](custom-pages.md) — Svelte views beyond `KTable` / `KForm`
