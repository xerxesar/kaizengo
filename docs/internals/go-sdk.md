...
The portable SDK lives in `packages/sdk-go` (appspec, acl, i18n, views, codegen). The **engine**, hook runtime, extension registry, and GQL guards live under **`internal/`** — in-tree apps import them today; they are host APIs, not the future remote SDK surface.
...
| `codegen` | Type and `.pot` generation (`kaizengo`) |
...
| `app` | Schema migrator + install helpers |
...
Each directory is `models/<model>/spec.yaml` plus optional Go (`hooks.go`, controllers).
...
| From spec | Runtime |
|---|---|
| `models[].fields` | Model table CRUD + `{app}Views` list/form metadata (tables via migrations) |
...
Schema defaults to app name (`KaizenGo_<APP>_SCHEMA` overrides).

### CQRS (commands & queries)

Declare the **public** surface in `app.yaml`. Models stay the persistence layer (`internal: true` recommended).
...
queries:
  - name: greetings
    list: greeting          # → hellospecGreetings
  - name: greeting
    get: greeting           # → hellospecGreeting(id)

commands:
  - name: postGreeting
    create: greeting        # → hellospecPostGreeting
  - name: reviseGreeting
    update: greeting
  - name: discardGreeting
    delete: greeting
```

Shorthand delegates to `ModelRegistry` (writes use `WithInternalWrite` so model field ACL still applies). Custom logic:

```go
module.Register(engine.New(engine.Options{
  AppName: "todo",
}).Command("completeAllTasks", func(c engine.HandlerCtx, _ map[string]any) (any, error) {
  return svc.CompleteAll(c.Context, c.OrgID, c.UserID)
}))
```
...
| Piece | Requirement |
|-------|-------------|
| `app.yaml` | Spec (models optional) |
| `security.yaml` (optional) | Listed under `security:`; roles, ACL entries, demo users |
| `migrations/001_*.sql` | Schema placeholder / bootstrap |
| `migrations/*.sql` | One per model (plural table name, e.g. `greetings`) |
...
| `//go:generate go run ../../cmd/kaizengo gen-types <name>` | In `module.go` |
...
```bash
go run ./cmd/kaizengo gen-types          # all apps
go run ./cmd/kaizengo gen-types identity # one app
```
...
There is **no auto schema sync**. Every app uses Postgres with explicit SQL migrations under `apps/<app>/migrations/`.

The server connects once via `internal/platform/postgres` and registers `platform.postgres` on the host. Apps call `postgres.FromHost(host)` (setup) or `postgres.FromContext(ctx)` (handlers). `engine.SetupEvents` uses that shared pool and a schema-scoped migrator without creating extra connections.

| App style | Migrations |
|-----------|------------|
| Spec-driven (`engine`) | `apps/<app>/migrations/*.sql` — applied at startup in sorted order |
| Hybrid (auth sessions) | Same directory; side tables + model tables |

Model-backed apps need:

1. Schema is created by the migrator (`EnsureBootstrap`)
2. **`00N_<plural>.sql`** — table per spec model (e.g. `greeting` → `greetings`, `acl_entry` → `acl_entries`)

Model tables share standard columns: `id`, `org_id`, `author_id`, `deleted`, `created_at`, `updated_at`, plus one column per model field (snake_case). The engine does not generate DDL from `app.yaml`.
...
`After*` runs after the row is written. `hc.Record` is the stored row; `hc.UserID` is set on create:
...
| `AfterCreate` | After write | final input | created row |
| `BeforeUpdate` | Before event append | mutable patch | previous row |
| `AfterUpdate` | After write | patch | updated row |
| `BeforeDelete` | Before event append | — | row being deleted |
| `AfterDelete` | After write | — | deleted row |
...
Use `kaizengo/internal/gql` for custom GraphQL guards.

## Environment variables

| Variable | Purpose |
|---|---|
| `KaizenGo_POSTGRES_DSN` | Postgres server connection; selected catalog DB name replaces the DSN path (`internal/platform/postgres`) |
| `KaizenGo_DATABASES_CONFIG` | Path to DB catalog JSON — names/selection/master hash only (default `.kaizengo/databases.json`) |
| `KaizenGo_<APP>_SCHEMA` | Per-app schema override (default = app name) |

## Generator

```bash
./bin/kaizengo new-app demo --type solid
```

Creates `app.yaml`, engine `module.go`, locales, migrations, and SPA stubs. Add `security:` / `security.yaml` entries for `{app}.{model}` if non-admin roles need access (admin already has `*`). See [ACL system](../acl.md).

See [cli.md](../cli.md) and [Workflow](../development/workflow.md).

For cross-app addons (search, audit, shared components), see [extension-platform.md](../extension-platform.md). For out-of-process providers (gRPC + go-plugin), see [grpc-plugins.md](../grpc-plugins.md).
