...
| `models` | GraphQL CRUD + `{app}Views` list/form metadata |
...
1. A bootstrap migration (legacy name `001_events.sql` is fine) — schema is created by the migrator
2. **`00N_<model>s_read.sql`** — table per spec model (table name: `{model}s_read`)
