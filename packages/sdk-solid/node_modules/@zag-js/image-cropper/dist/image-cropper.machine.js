...
var import_crop = require("./utils/crop.js");
var machine = (0, import_core.createMachine)({
  ...
  context({ bindable, prop }) {
    return {
      ...
      offset: bindable(() => ({
        defaultValue: import_crop.ZERO_POINT
      })),
  ...
  computed: {
    isMeasured: ({ context }) => (0, import_crop.isVisibleRect)(context.get("viewportRect")) && (0, import_crop.isVisibleRect)(context.get("crop")),
    isImageReady: ({ context }) => (0, import_crop.isVisibleRect)(context.get("naturalSize"))
  },
  ...
  implementations: {
    guards: {
      hasViewportRect({ context }) {
        return (0, import_crop.isVisibleRect)(context.get("viewportRect"));
      },
      canResizeCrop({ context, prop }) {
        return !prop("fixedCropArea") && (0, import_crop.isVisibleRect)(context.get("viewportRect"));
      },
      canPan({ context }) {
        return (0, import_crop.isVisibleRect)(context.get("naturalSize")) && (0, import_crop.isVisibleRect)(context.get("viewportRect"));
      },
      canDragSelection({ context, prop }) {
        return (0, import_crop.isVisibleRect)(context.get("viewportRect")) && !prop("fixedCropArea");
      }
    },
    actions: {
      checkImageStatus({ send, scope, context }) {
        ...
        if ((0, import_crop.isVisibleRect)({ width, height }) && !(0, import_crop.isVisibleRect)(naturalSize)) {
      ...
      setDefaultCrop({ context, prop, scope }) {
        ...
        if (!(0, import_crop.isVisibleRect)(viewportRect)) return;
        const cropShape = prop("cropShape");
        const aspectRatio = (0, import_crop.resolveCropAspectRatio)(cropShape, prop("aspectRatio"));
        const { minSize, maxSize } = (0, import_crop.getCropSizeLimits)(prop);
        const clampSize = (rect) => {
          const result = (0, import_crop.computeResizeCrop)({
            cropStart: rect,
            handlePosition: "se",
            delta: import_crop.ZERO_POINT,
            viewportRect,
            minSize,
            maxSize,
            aspectRatio
        ...
        if (initialCrop) {
          ...
          const max = (0, import_crop.getMaxBounds)({ width: width2, height: height2 }, viewportRect);
          const { x: x2, y: y2 } = (0, import_crop.clampPoint)(initialCrop, import_crop.ZERO_POINT, max);
          context.set("crop", { x: x2, y: y2, width: width2, height: height2 });
          return;
        }
        const fixedCropArea = prop("fixedCropArea");
        const defaultSize = (0, import_crop.computeDefaultCropDimensions)(viewportRect, aspectRatio, fixedCropArea);
        ...
        const { x, y } = (0, import_crop.centerRect)({ width, height }, viewportRect);
      ...
      updateCrop({ context, event, prop }) {
        ...
        let aspectRatio = (0, import_crop.resolveCropAspectRatio)(cropShape, aspectRatioProp);
        const { minSize, maxSize } = (0, import_crop.getCropSizeLimits)(prop);
        if (!pointerStart || !cropStart) return;
        const currentPoint = event.point;
        let delta = (0, import_crop.subtractPoints)(currentPoint, pointerStart);
        let nextCrop;
        if (handlePosition) {
          ...
          nextCrop = (0, import_crop.computeResizeCrop)({
            cropStart,
            handlePosition,
            delta,
            viewportRect,
          ...
          if (event.altKey) {
            const originalCenter = (0, import_crop.getCenterPoint)(cropStart);
            const pos = (0, import_crop.centerCropOnPoint)(nextCrop, originalCenter, viewportRect);
            nextCrop = { ...nextCrop, x: pos.x, y: pos.y };
          }
        } else {
          nextCrop = (0, import_crop.computeMoveCrop)(cropStart, delta, viewportRect);
        }
      ...
      updatePanOffset({ context, event, prop }) {
        ...
        const viewportRect = context.get("viewportRect");
        const delta = (0, import_crop.subtractPoints)(point, pointerStart);
        const nextOffset = (0, import_crop.clampOffset)({
          zoom,
          rotation,
          viewportSize: viewportRect,
          offset: (0, import_crop.addPoints)(offsetStart, delta),
      ...
      setFlip({ context, event }) {
        ...
        const normalized = (0, import_crop.normalizeFlipState)(nextFlip, currentFlip);
        if ((0, import_crop.isEqualFlip)(normalized, currentFlip)) return;
      ...
      resizeCrop({ context, event, prop }) {
        ...
        const { minSize, maxSize } = (0, import_crop.getCropSizeLimits)(prop);
        const crop = context.get("crop");
        const nextCrop = (0, import_crop.computeResizeCrop)({
          cropStart: crop,
          handlePosition,
          delta,
          viewportRect,
      ...
      updateZoom({ context, event, prop }) {
        ...
        const fixedCropArea = prop("fixedCropArea");
        if (!point) {
          point = (0, import_crop.getCenterPoint)(crop);
        }
        ...
        const applyClampedOffset = (zoom, offset) => {
          return (0, import_crop.clampOffset)({
            zoom,
            rotation,
            viewportSize: viewportRect,
            offset,
        ...
        if (nextZoom === currentZoom && panDelta) {
          const nextOffset2 = applyClampedOffset(currentZoom, (0, import_crop.addPoints)(currentOffset, panDelta));
          context.set("offset", nextOffset2);
          return;
        }
        if (nextZoom === currentZoom) return;
        const { width: viewportWidth, height: viewportHeight } = viewportRect;
        const { x: centerX, y: centerY } = (0, import_crop.getViewportCenter)(viewportRect);
        ...
        if (panDelta) {
          nextOffset = applyClampedOffset(nextZoom, (0, import_crop.addPoints)(nextOffset, panDelta));
        } else if (nextZoom < currentZoom) {
          if (fixedCropArea) {
            nextOffset = applyClampedOffset(nextZoom, nextOffset);
          } else {
            const { width: scaledImageWidth, height: scaledImageHeight } = (0, import_crop.scaleSize)(viewportRect, nextZoom);
      ...
      setPinchDistance({ context, event, send }) {
        ...
        const distance = (0, import_crop.getTouchDistance)(first, second);
        const viewportRect = context.get("viewportRect");
        const midpoint = (0, import_crop.getMidpoint)(first, second, { x: viewportRect.left, y: viewportRect.top });
      ...
      handlePinchMove({ context, event, send }) {
        ...
        if (lastDistance != null && lastDistance > 0 && lastMidpoint != null) {
          ...
          const hasSignificantZoom = distanceChange > import_crop.MIN_PINCH_DISTANCE;
          const panDelta = (0, import_crop.subtractPoints)(midpoint, lastMidpoint);
          send({
            type: "ZOOM",
            trigger: "touch",
            delta,
      ...
      nudgeResizeCrop({ context, event, prop }) {
        ...
        const crop = context.get("crop");
        const viewportRect = context.get("viewportRect");
        const step = (0, import_crop.getNudgeStep)(prop, { shiftKey, ctrlKey, metaKey });
        const { minSize, maxSize } = (0, import_crop.getCropSizeLimits)(prop);
        const nextCrop = (0, import_crop.computeKeyboardCrop)(key, handlePosition, step, crop, viewportRect, minSize, maxSize);
        context.set("crop", nextCrop);
      },
      nudgeMoveCrop({ context, event, prop }) {
        ...
        const viewportRect = context.get("viewportRect");
        const step = (0, import_crop.getNudgeStep)(prop, { shiftKey, ctrlKey, metaKey });
        const delta = (0, import_crop.getKeyboardMoveDelta)(key, step);
        const nextCrop = (0, import_crop.computeMoveCrop)(crop, delta, viewportRect);
        context.set("crop", nextCrop);
      },
      nudgePan({ context, event, prop }) {
        ...
        const nextOffset = (0, import_crop.clampOffset)({
          ...
          rotation,
          viewportSize: viewportRect,
          offset: (0, import_crop.addPoints)(context.get("offset"), delta),
          fixedCropArea: prop("fixedCropArea"),
          crop: context.get("crop")
        });
        context.set("offset", nextOffset);
      ...
      resizeViewport({ context, prop, scope, send }) {
        ...
        if (!(0, import_crop.isVisibleRect)(newViewportRect)) return;
        const oldViewportRect = context.get("viewportRect");
        if ((0, import_crop.isSameSize)(oldViewportRect, newViewportRect)) {
          return;
        }
        context.set("viewportRect", newViewportRect);
        const oldCrop = context.get("crop");
        if (!(0, import_crop.isVisibleRect)(oldCrop)) {
          send({ type: "SET_DEFAULT_CROP", src: "viewport-resize" });
          return;
        }
        if (!(0, import_crop.isVisibleRect)(oldViewportRect)) {
          return;
        }
        const cropShape = prop("cropShape");
        const aspectRatio = (0, import_crop.resolveCropAspectRatio)(cropShape, prop("aspectRatio"));
        ...
        let newCrop = (0, import_crop.scaleRect)(oldCrop, scale);
        const constrainedCrop = (0, import_crop.computeResizeCrop)({
          cropStart: newCrop,
          handlePosition: "se",
          delta: import_crop.ZERO_POINT,
        ...
        const max = (0, import_crop.getMaxBounds)(constrainedCrop, newViewportRect);
        const { x, y } = (0, import_crop.clampPoint)(constrainedCrop, import_crop.ZERO_POINT, max);
        context.set("crop", {
          x,
          y,
      ...
      resetToInitialState({ context, send }) {
        ...
        context.set("offset", import_crop.ZERO_POINT);
      ...
      adjustCropAspectRatio({ context, prop }) {
        ...
        if (!(0, import_crop.isVisibleRect)(viewportRect)) return;
        const crop = context.get("crop");
        if (!(0, import_crop.isVisibleRect)(crop)) return;
        const cropShape = prop("cropShape");
        const aspectRatio = (0, import_crop.resolveCropAspectRatio)(cropShape, prop("aspectRatio"));
        if (aspectRatio === void 0) return;
        const currentAspect = crop.width / crop.height;
        if ((0, import_crop.isAspectRatioEqual)(currentAspect, aspectRatio)) return;
        const { minSize, maxSize } = (0, import_crop.getCropSizeLimits)(prop);
        const constrainedCrop = (0, import_crop.computeResizeCrop)({
          cropStart: crop,
          handlePosition: "se",
        ...
        if ((0, import_crop.isSameSize)(crop, constrainedCrop)) return;
        const center = (0, import_crop.getCenterPoint)(crop);
        const pos = (0, import_crop.centerCropOnPoint)(constrainedCrop, center, viewportRect);
        context.set("crop", {
          x: pos.x,
          y: pos.y,
          width: constrainedCrop.width,
